...
extension MarkdownEditorView: UITextViewDelegate {
  func textView(_ textView: UITextView, shouldChangeTextIn range: NSRange, replacementText text: String) -> Bool {
    // 1. 현재 rawText와 indexMap을 기반으로 변경될 범위를 찾습니다.
    let rawStart: Int
    ...
    // 3. 변경된 rawText로 뷰를 다시 렌더링합니다.
    applyMarkdownStyle()

    // 4. 렌더링 후, 새로운 텍스트와 매핑 테이블을 기반으로 커서 위치를 업데이트합니다.
    ...
    // 5. UITextView가 자체적으로 텍스트를 변경하는 것을 막습니다.
    // 모든 변경은 applyMarkdownStyle()을 통해 이루어집니다.
    return false
  }

  func textViewDidChange(_ textView: UITextView) {
    // 'shouldChangeTextIn'에서 모든 로직을 처리하므로 이 메소드는 비워둡니다.
    // 만약 'shouldChangeTextIn'에서 처리할 수 없는 경우 (예: 컷/붙여넣기, 자동 완성 등)
    // 이곳에서 동기화 로직을 구현해야 하지만, 현재 구현에서는 비워둡니다.
  }
}
