...
> 찾아보니 애플 로그인은 그 이메일을 완전히 첫 시도 때만 뿌려줘서 같은 주소로 반복한 내 계정으로는 안 나오는 것이 당연했고, 다른 계정으로 로그인을 시도하니 제대로 되었다.
>
>  이 문제는 회원가입을 하다가 중도 취소 후 다시 시도할 때 문제가 되었다. 해결 방법은 애플 로그인을 최초 시도할 때 키체인에 저장을 하고, 회원가입이 종료되면 지우는 방식으로 구현했다.
