...
> 컬렉션뷰의 헤더에 데이터를 넣어야 하는 상황이었습니다. 핀포인트 기능을 위해 헤더가 아닌 셀로 교체가 어려운 상황이었습니다. 그런데 컬렉션뷰의 SupplementaryRegistration는 외부 데이터를  지원하지 않아, 외부 변수에서 참조를 통해 데이터를 넣어줘 해결하였습니다. 그러나 이 문제 때문에  DiffableDataSource의 apply로 헤더가 갱신이 되지 않았습니다.
